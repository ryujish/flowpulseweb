...
export default function StockAnalysis({stocks=[],candidateMeta,observedCodes=[],favoriteCodes=[],onWatch,onOpenWatch,onFavorite}:{stocks?:SearchableStock[];candidateMeta?:{universeCount:number;priceCount:number;calculatedAt?:string};observedCodes?:string[];favoriteCodes?:string[];onWatch?:(stock:SearchableStock)=>void;onOpenWatch?:()=>void;onFavorite?:(stock:SearchableStock)=>void}) {
  ...
  const priceText=(stock:SearchableStock)=>stock.market.includes("미국")||["나스닥","뉴욕","아멕스"].some(name=>stock.market.includes(name))?`$${stock.price.toLocaleString("en-US")}`:`${stock.price.toLocaleString("ko-KR")}원`,
    ...
    chartRows=(key:Exclude<keyof DetailPoint,"time">)=>{const rows=chartPoints.flatMap((point,index)=>point[key]===null?[]:[{index,value:Number(point[key])}]),values=rows.map(row=>row.value),min=Math.min(...values),max=Math.max(...values),span=max-min||1;return rows.map(row=>({...row,x:55+row.index*665/Math.max(1,chartPoints.length-1),y:295-(row.value-min)*240/span}))},
    chartPath=(key:Exclude<keyof DetailPoint,"time">)=>chartRows(key).map((row,index)=>`${index?"L":"M"}${row.x} ${row.y}`).join(" "),
    changeMarkers=(key:"foreign"|"institution")=>chartRows(key).filter((row,index,rows)=>index===0||row.value!==rows[index-1].value),
    ...
    selectedPlaybook=playbooks[selected.playbook??""]??["KIS-FPA","점수 계산 대기"],
    fpaConditions=[(selected.fpaScore??0)>=75,(selected.programScore??0)>=60,(selected.liquidityScore??0)>=60,(selected.dataConfidence??0)>=75,(selected.riskPenalty??99)<20],
    metConditions=fpaConditions.filter(Boolean).length,
    confidenceLabel=(selected.dataConfidence??0)>=90?"매우 높음":(selected.dataConfidence??0)>=75?"높음":(selected.dataConfidence??0)>=60?"보통":"추천 제외";
  return <div className="stock-analysis" ref={captureRef}>
    ...
    <section className="stock-surface playbook-summary"><header><div><small>KIS-FPA · v1.0</small><h2>{selected.name} · {selected.grade??"점수 계산 대기"}</h2></div><span>{selectedPlaybook[1]}</span></header><p>{selected.ambiguous?"추세형과 반전형 점수 차이가 5점 미만으로 방향을 확인하고 있습니다.":`${selectedPlaybook[1]} · 프로그램 수급과 가격 조건을 함께 확인합니다.`}</p><div className="playbook-metrics"><div><span>KIS-FPA</span><strong>{selected.fpaScore??"-"} / 100</strong></div><div><span>프로그램</span><strong>{selected.programScore??"-"} / 100</strong></div><div><span>외국인 추정</span><strong>{selected.foreignScore??"-"} / 100</strong></div><div><span>기관 추정</span><strong>{selected.institutionScore??"-"} / 100</strong></div><div><span>유동성</span><strong>{selected.liquidityScore??"-"} / 100</strong></div><div><span>데이터 신뢰도</span><strong>{selected.dataConfidence??"-"} · {confidenceLabel}</strong></div></div><div className="condition-progress"><div><span>추천 게이트 5개 중 {metConditions}개 충족</span><b>위험 감점 {selected.riskPenalty??"-"} · 확인 보너스 +{selected.confirmBonus??0}</b></div><progress max="5" value={metConditions}/></div><small>데이터 신뢰도는 상승 또는 수익 확률이 아니라 입력 데이터의 품질입니다.</small><div className="playbook-actions"><button onClick={()=>onFavorite?.(selected)}>{favoriteCodes.includes(selected.code)?"관심 등록됨":"일반 관심 등록"}</button><button onClick={()=>startObservation(selected)}>{observed?"관찰 중 · 관심 종목 보기":"관찰 시작"}</button></div></section>
    ...
    {view === "판단 근거" && <><section className="stock-surface judgment-summary"><span>KIS-FPA 판정</span><b>{selected.grade??"점수 계산 대기"}</b><p>추세 {selected.trendScore??"-"} · 반전 {selected.reversalScore??"-"} · 높은 전략은 {selectedPlaybook[1]}입니다.</p><small>{asOfText} 기준 · 외국인·기관은 KIS 추정 집계</small></section><section className="stock-surface condition-table"><h2>추천 게이트</h2>{[["KIS-FPA 75 이상",fpaConditions[0],selected.fpaScore],["프로그램 60 이상",fpaConditions[1],selected.programScore],["유동성 60 이상",fpaConditions[2],selected.liquidityScore],["데이터 신뢰도 75 이상",fpaConditions[3],selected.dataConfidence],["위험 감점 20 미만",fpaConditions[4],selected.riskPenalty]].map(([label,met,value])=><div key={String(label)}><b>{label}</b><span className={met?"met":"miss"}>{met?"충족":"미충족"}</span><small>현재 {value??"수집 중"}</small></div>)}</section><section className="evidence-grid"><article className="stock-surface positive"><h2>확인된 근거</h2>{(selected.programScore??0)>=60&&<div><span>프로그램</span><strong>{selected.programScore}점</strong></div>}{(selected.foreignScore??0)>=60&&<div><span>외국인 추정</span><strong>{selected.foreignScore}점</strong></div>}{(selected.institutionScore??0)>=60&&<div><span>기관 추정</span><strong>{selected.institutionScore}점</strong></div>}{(selected.liquidityScore??0)>=60&&<div><span>유동성</span><strong>{selected.liquidityScore}점</strong></div>}</article><article className="stock-surface opposing"><h2>제한 근거</h2>{selected.ambiguous&&<div><span>전략 방향</span><strong>점수 차이 5점 미만</strong></div>}{(selected.programScore??0)<60&&<div><span>프로그램</span><strong>기준 미달</strong></div>}{(selected.dataConfidence??0)<75&&<div><span>데이터</span><strong>신뢰도 제한</strong></div>}{(selected.riskPenalty??0)>0&&<div><span>위험 감점</span><strong>-{selected.riskPenalty}</strong></div>}</article></section><section className="stock-surface criteria"><h2>KIS-FPA 판단 기준 · v1.0</h2><div><b>추천</b><span>FPA 75 · 프로그램 60 · 유동성 60 · 신뢰도 75</span></div><div><b>관찰</b><span>FPA 60~74 또는 전략 점수 차이 5점 미만</span></div><div><b>제외</b><span>FPA 60 미만 또는 위험 감점 20 이상</span></div><button onClick={()=>startObservation(selected)}>관찰 시작</button></section></>}

    {view === "가격·수급 차트" && <><section className="stock-surface price-flow-card"><div className="stock-section-head"><div><h2>가격과 투자 주체별 수급</h2><small>가격·프로그램은 1분선 · 외국인·기관은 장중 추정 갱신점 · 계열별 정규화</small></div><div className="range-tabs">{["5분","15분","30분","당일"].map(item=><button className={range===item?"active":""} onClick={()=>setRange(item)} key={item}>{item}</button>)}</div></div><div className="stock-chart-legend"><span className="price">━ 가격</span><span className="foreign">● 외국인 추정</span><span className="institution">● 기관 추정</span><span className="program">━ 프로그램</span><span>개인 · 미제공</span></div><svg className="price-flow-chart" viewBox="0 0 760 330" role="img" aria-label={`${selected.name} ${range} 가격과 투자 주체별 수급 차트`}>{[55,115,175,235,295].map(y=><line key={y} x1="55" x2="720" y1={y} y2={y} className="grid"/>)}{chartTicks.map((point,index)=>{const x=55+chartPoints.indexOf(point)*665/Math.max(1,chartPoints.length-1);return <g key={point.time}><line x1={x} x2={x} y1="35" y2="295" className="grid"/><text x={x} y="320" textAnchor={index===0?"start":index===chartTicks.length-1?"end":"middle"}>{new Date(point.time).toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit",hour12:false})}</text></g>})}<path className="price" d={chartPath("price")}/><path className="program" d={chartPath("program")}/>{changeMarkers("foreign").map(row=><circle key={`f-${row.index}`} cx={row.x} cy={row.y} r="5" fill="#20a589"/>)}{changeMarkers("institution").map(row=><circle key={`i-${row.index}`} cx={row.x} cy={row.y} r="5" fill="#c94b8c"/>)}</svg>{!chartPoints.length&&<p className="analysis-copy">종목별 1분 데이터를 수집하고 있습니다.</p>}<div className="flow-stats"><div><span>Flow Shift</span><strong>{bullish?"매수 유지":"순매도"}</strong></div><div><span>최근 5분 프로그램</span><strong>{deltaAt(detailPoints,"program",5)===null?"수집 대기":programText(deltaAt(detailPoints,"program",5)!)}</strong></div><div><span>데이터</span><strong>{chartPoints.length}개</strong></div><div><span>상태</span><strong>{verdict}</strong></div></div></section><section className="stock-surface stock-check"><div><h2>지금 확인할 것</h2><p>{verdictCopy}</p></div><dl><div><dt>현재가</dt><dd>{priceText(selected)}</dd></div><div><dt>최근 갱신</dt><dd>{asOfText}</dd></div></dl></section></>}
    </>}
