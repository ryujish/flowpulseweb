...
function MarketDashboard({
  ...
  const actors = snapshot
    ...
    signalCopy = (stock: typeof samsung) => !stock ? "실데이터를 수집하고 있습니다." : `08:00 이후 프로그램 ${direction(stock.program)} ${formatEok(Math.abs(stock.program),false)}억원 · 외국인 ${stock.investorAvailable?`${direction(stock.foreign)} ${Math.abs(stock.foreign).toLocaleString("ko-KR")}주`:stockInvestorPending}`;
  return (
          ...
          <div className="flow-cards">
            ...
            {(data?.leaders ?? []).map((stock) => market === "NASDAQ" ? (
              ...
              <article key={stock.code} className={`leader-flow ${stock.changeRate >= 0 ? "stock-up" : "stock-down"}`}>
                ...
                <div><span>개인<b>{stock.investorAvailable?signedShares(stock.personal):stockInvestorPending}</b></span><span>외국인<b>{stock.investorAvailable?signedShares(stock.foreign):stockInvestorPending}</b></span><span>기관<b>{stock.investorAvailable?signedShares(stock.institution):stockInvestorPending}</b></span><span>프로그램<b>{signed(stock.program)}</b></span></div>
              </article>
            ))}
          ...
          {market === "KOSPI" && <section className="fp-signals">
            ...
            <div className="fp-signal-grid">
              <article className="fp-signal-card">
                ...
                <dl><div><dt>현재가</dt><dd>{stockPrice(samsung,"수집 중")}</dd></div><div><dt>프로그램</dt><dd className={samsung?.program! >= 0 ? "positive" : "negative"}>{samsung ? direction(samsung.program) : "수집 중"}</dd></div><div><dt>기관</dt><dd>{samsung?.investorAvailable ? direction(samsung.institution) : stockInvestorPending}</dd></div></dl>
              ...
              <article className="fp-signal-card caution">
                ...
                <dl><div><dt>현재가</dt><dd>{stockPrice(hynix,"수집 중")}</dd></div><div><dt>프로그램</dt><dd className={hynix?.program! >= 0 ? "positive" : "negative"}>{hynix ? direction(hynix.program) : "수집 중"}</dd></div><div><dt>기관</dt><dd>{hynix?.investorAvailable ? direction(hynix.institution) : stockInvestorPending}</dd></div></dl>
                <footer>08:00 첫 수집값을 0으로 두고 이후 변화량을 표시합니다.</footer>
              </article>
            </div>
          </section>}
